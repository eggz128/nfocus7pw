...
test('LoginLogoutTest', async ({ page }) => {
  ...
  */
  ...
  //Chining locators works for Playwright as it does in WebDriver (and Cypress...)
  ...
  */
  ...
  //cy.get('#password').type('edgewords'); //ToDo: check if Cypress auto clears
  ...
  //WD Equivalent: driver.SwitchTo().Alert().Accept() //Note you would handle the alert at after it spawned. In PW you set up handling *before* it spawns.
  
  /*
  *Assert - did Login/Logout work?
  */
  
  //Assertion doesnt capture value to test - instead acts on locator
